...
def main(start, end):
    ...
    result_file_path = "perf_data_final/perf_first_lrzip.csv" # change subject by change perf_first_[SubjectName].csv
    ...
    #np.random.seed(0)
    all_data = pd.read_csv(result_file_path, index_col=0)
    ...
    for count in range(end, end+1):
        ...
        weight, config_blacklist, bias_point, stop_point, new_mix_result, sampled_config_ids = process_limited.comsa(config, init_idx, weight, all_data,
...
if __name__ == "__main__":
    commit_number = 60
    for i in range(0, commit_number):
